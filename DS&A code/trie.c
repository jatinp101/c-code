 ...
 * Helper functions
 * */
void helper_traverse_nodes(struct trie_node *root, char word[], int length);
void helper_print_word(struct trie_node *root, char word[], int length);
...
void auto_comp(struct trie_node *root, char word[], int length)
{
    helper_traverse_nodes(root, word, length);
}

void helper_traverse_nodes(struct trie_node *root, char word[], int length){
    if (!root) return;   
    helper_print_word(root, word, length);
    for(int i =0; i < 26; i++){
        helper_traverse_nodes(root->child[i], word, length);
    }
    return;
}
...
void helper_print_word(struct trie_node *root, char word[], int length){
    ...
    for(int i = 0; i < length; i++) {
        if(root->word[i] != word[i]) return;
    }
    for(int i = 0; i < 26;i++) {
        if(root->word[i] == '\0') break;
